...
功能 CountDown_初始化()
    ...
    变量 event = 事件打开("防多开")
    如果(event)
        对话框("已经有相同的CountDowner实例在运行!","Matrix Box CountDowner",0,真)
        退出()
    否则
        事件创建(假,真,"防多开")
    结束
    ...
    //读配置
    设置托盘("Matrix Box CountDowner",假)
    设置托盘气泡("欢迎使用Matrix Box CountDowner Beta 2,仍处在测试中","Matrix Box CountDowner")
...
功能 定时器0_时钟()
    ...
    如果(displayday <= 0   )
        ...
        设置托盘气泡("您所设定的时间已经到达,计时器将停止,请重新设置计时器后重启程序","Matrix Box CountDowner")
...
功能 按钮4_点击()
    //这里添加你要执行的代码
    对话框("Matrix Box CountDowner 0.2 Beta 2","关于",0,假)
结束
